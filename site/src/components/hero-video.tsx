...
export function HeroVideo({ videoSrc }: HeroVideoProps) {
  ...
  return (
    ...
    >
      {reduce ? (
        <div className="absolute inset-0">{videoLayer}</div>
      ) : (
      ...
      {reduce ? (
        <div className="relative z-10 mx-auto grid max-w-6xl items-center gap-10 px-4 py-16 md:px-8 lg:grid-cols-[1fr_auto] lg:py-20">
          <HeroBody />
        </div>
      ) : (
        <motion.div
      ...
      >
        ...
        {reduce ? (
          <span className="h-8 w-5 rounded-full border-2 border-white/30">
    ...
    <>
      <div className="text-center lg:text-left">
        <h1 className="font-[family-name:var(--font-heading)] text-4xl leading-[1.08] font-bold tracking-tight text-balance text-white sm:text-5xl lg:text-6xl">
          Controle de pragas com método e documentação técnica
        </h1>
        <p className="mx-auto mt-6 max-w-xl text-lg leading-relaxed text-pretty text-white/85 lg:mx-0">
          ...
          pedem resposta rápida e registro adequado. Chame no WhatsApp e reduza
          risco à saúde e autuações relacionadas a pragas vetores.
        </p>
      ...
      {reduce ? (
        <div className="mx-auto w-full max-w-sm rounded-2xl bg-white/95 px-6 py-8 shadow-2xl ring-1 ring-white/30 backdrop-blur-sm lg:max-w-xs">
          <LogoBrandFull
