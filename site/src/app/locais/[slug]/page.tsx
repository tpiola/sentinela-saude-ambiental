...
export async function generateMetadata({params}:Props):Promise<Metadata>{const {slug}=await params;const page=locations.find(x=>x.slug===slug);if(!page)return{};return{title:page.title,description:page.description,alternates:{canonical:`/locais/${slug}`},openGraph:{title:page.title,description:page.description,url:`/locais/${slug}`}};}
