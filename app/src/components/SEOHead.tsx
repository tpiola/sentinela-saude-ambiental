...
const GA_CONFIG_SNIPPET = (measurementId: string) =>
...
export default function SEOHead() {
  useEffect(() => {
    ...
    const appendToHead = (el: HTMLElement) => {
    ...
    if (gtmId?.trim()) {
      ...
      gtmScript.textContent = GTM_SNIPPET(gtmId.trim());
      appendToHead(gtmScript);
    }
    ...
    if (gaId?.trim()) {
      ...
      gaScript.async = true;
      gaScript.src = `https://www.googletagmanager.com/gtag/js?id=${encodeURIComponent(gaId.trim())}`;
      appendToHead(gaScript);

      const gaConfig = document.createElement('script');
      gaConfig.textContent = GA_CONFIG_SNIPPET(gaId.trim());
      appendToHead(gaConfig);
    }
      ...
      '@graph': [
        {
          ...
          description:
            'Controle preventivo de pragas urbanas em Franca SP. Escorpiões, baratas, cupins, roedores, higienização de reservatórios.',
        ...
        },
        {
          ...
          mainEntity: [
            ...
            },
            {
              ...
              acceptedAnswer: {
                '@type': 'Answer',
                text: 'De outubro a março é o período crítico em Franca e interior paulista. SP registrou 42.526 casos em 2025. A proteção preventiva deve começar em setembro.',
              },
            },
            {
              '@type': 'Question',
    ...
    const metaTags: Record<string, string>[] = [
      {
        name: 'keywords',
        content:
          "dedetização Franca SP, controle de pragas, escorpião, baratas, cupim, roedores, caixa d'água, sanitização, desratização",
      },
      { property: 'og:title', content: 'Sentinela Saúde Ambiental | Controle de Pragas Franca SP' },
      {
        property: 'og:description',
        content: 'Proteção técnica contra pragas em Franca SP. Diagnóstico gratuito em minutos.',
      },
      { property: 'og:type', content: 'website' },
      { property: 'og:url', content: 'https://sentinelasaudeambiental.com.br' },
      {
        property: 'og:image',
        content: 'https://sentinelasaudeambiental.com.br/hero-casa-protegida.jpg',
      },
